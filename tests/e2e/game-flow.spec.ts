...
test("starts a game, wins, and writes one leaderboard score", async ({ page }) => {
  await page.goto("/");
  await page.getByLabel("Your name").fill("Browser Player");
  await page.getByLabel("Choose your range").selectOption("10");
  await page.getByRole("button", { name: "Start a new game" }).click();

  await expect(page).toHaveURL(/\/game$/);
  await expect(page.getByRole("heading", { name: /Good luck, Browser Player/ })).toBeVisible();
  ...
  await expect(page.getByRole("cell", { name: "Browser Player" })).toBeVisible();
